...
export const namespaceKeys = (namespace: string, obj: Record<string, any>): Record<string, any> => {
  return namespace === ''
    ? obj
    : mapObjKeys((key: String) => `${namespace}.${key}`, obj)
}

export const pathInParent2ObjectPath = (pathInParent: string[]): string[] => pathInParent.map(k => ['states', k]).flat()

export const submachines = (machineDef: TMachineDefOrStateDef, parentPath: string[] = []): TFoundSubmachine[] => {
  let result:TFoundSubmachine[]   = []
  if (machineDef.states) {
    for (const [stateKey, stateDef] of Object.entries(machineDef.states)) {
      if (stateDef.states) {
        result = result.concat(submachines(stateDef, parentPath.concat([stateKey])))
